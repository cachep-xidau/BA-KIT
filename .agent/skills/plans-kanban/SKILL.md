...
cd .claude/skills/ck:plans-kanban
npm install
```
...
node .claude/skills/ck:plans-kanban/scripts/server.cjs \
  --dir ./ck:plans \
  --open
...
node .claude/skills/ck:plans-kanban/scripts/server.cjs --stop
```
...
| `/api/ck:plans` | JSON API for plans data |
| `/api/ck:plans?dir=<path>` | JSON API for specific directory |
...
{
  "success": true,
  "url": "http://localhost:3500/ck:kanban?dir=...",
  "networkUrl": "http://192.168.2.75:3500/ck:kanban?dir=...",
  "port": 3500
}
```
...
**PID files**: Located at `/tmp/ck:plans-kanban-*.pid`

...
def run_basic_test(url: str, take_screenshot: bool = False) -> dict:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Screenshot - JPEG viewport-only to minimize file size & token usage
            if take_screenshot:
                screenshot_dir = os.path.join(tempfile.gettempdir(), "maestro_screenshots")
                os.makedirs(screenshot_dir, exist_ok=True)
                screenshot_path = os.path.join(screenshot_dir, f"screenshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpeg")
                page.set_viewport_size({"width": 1440, "height": 900})
                page.screenshot(path=screenshot_path, full_page=False, type="jpeg", quality=80)
                result["screenshot"] = screenshot_path
                result["screenshot_note"] = "JPEG viewport-only (optimized for token efficiency)"

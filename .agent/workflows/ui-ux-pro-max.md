...
- **Figma link**: If provided, fetch design specs via MCP

### Step 1.5: Figma Design Input (If Provided)

If user provides a Figma link, fetch design data first:

```bash
# Parse URL: https://www.figma.com/design/FILE_KEY/NAME?node-id=NODE_ID
# Extract FILE_KEY and NODE_ID

# Fetch via stdin piping (structured data - preferred, low token cost)
...
> ⚠️ **Image Export**: If you need screenshots, use `format: "jpg"` and `scale: 0.5`.
> Resize with `sips -Z 1200` before reading. Large PNGs crash the context window.
> Prefer `get_file_nodes` (JSON) over images whenever possible.
